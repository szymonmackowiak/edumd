...
def update_verlet_list_old(RX, RY, RZ, PBC, DBOX, RCUT, SKIN, particles_to_update, neighbouring_particles, v_list, marker):
    ...
    for item in particles_to_update:
    
        v_row = []
        
        for jtem in neighbouring_particles:
            ...
            if ((R2<((RCUT+SKIN)*(RCUT+SKIN))) and (item!=jtem)):
                v_row.append(jtem)
                
        v_list[item] = v_row
        marker[item] = len(v_row)
    
    return v_list, marker

def update_verlet_list(RX, RY, RZ, PBC, DBOX, RCUT, SKIN, particles_to_update, neighbouring_particles, v_list, marker):
    XPERIOD, YPERIOD, ZPERIOD = PBC
    BX, BY, BZ = DBOX

    for item in particles_to_update:
        v_row = set()  # Używamy zbioru, by uniknąć duplikatów
        
        for jtem in neighbouring_particles:
            if item == jtem:
                continue  # Pomijamy samą cząsteczkę
            
            X = RX[item] - RX[jtem]
            Y = RY[item] - RY[jtem]
            Z = RZ[item] - RZ[jtem]
            
            # Okresowe warunki brzegowe (PBC)
            X -= XPERIOD * BX * round(X / BX)
            Y -= YPERIOD * BY * round(Y / BY)
            Z -= ZPERIOD * BZ * round(Z / BZ)
        ...
        v_list[item] = list(v_row)  # Konwersja zbioru na listę
        marker[item] = len(v_row)
...
def old_forces_lj_full_interaction(RX, RY, RZ, PBC, DBOX, RCUT):
...
def newton_verlet_leap_frog_pbc(RX, RY, RZ, VXH2, VYH2, VZH2, FX, FY, FZ, PBC, DBOX, DT, SKIN):
...
def grid_structure(DBOX, INTRANGE):
    
    BX, BY, BZ = DBOX[0], DBOX[1], DBOX[2]
                    ...
                    for jj in [-1, 0, 1]:
                        for ii in [-1, 0, 1]:
                            if ii == 0 and jj == 0 and kk == 0:
                                ...
                                idn = i_x + i_y * NX + i_z * NX * NY
                        
                                NEIGHBOURS.append(idn)
                    ...
                    for jj in [-2, -1, 0, 1, 2]:
                        for ii in [-2, -1, 0, 1, 2]:
                            if ii == 0 and jj == 0 and kk == 0:
                                ...
                                NEIGHBOURS_2.append(idn)
                                
                                
                
                NEIGHBOURS_LIST.append(NEIGHBOURS)
                NEIGHBOURS_LIST_2.append(NEIGHBOURS_2)
                counter = counter + 1
    
    return counter, [x0, y0, z0], [NX, NY, NZ], [dim_x, dim_y, dim_z], NEIGHBOURS_LIST, NEIGHBOURS_LIST_2

#------------------------------------------------------------------------------

def grid_content(RX, RY, RZ, grid_N, grid_dim, grid_origin):
    NX, NY, NZ = grid_N[0], grid_N[1], grid_N[2]
    x0, y0, z0 = grid_origin[0], grid_origin[1], grid_origin[2]
    dim_x, dim_y, dim_z = grid_dim[0], grid_dim[1], grid_dim[2]
    
    where_is_parcicle = []
    cell_content = [[] for _ in range(NX*NY*NZ)]
    
    for i in range(0, len(RX)):
        
        index_x = int((RX[i] - x0)/dim_x)
        index_y = int((RY[i] - y0)/dim_y)
        index_z = int((RZ[i] - z0)/dim_z)
        
        idx = index_x + index_y * NX + index_z * NX * NY
        cell_content[idx].append(i)
        where_is_parcicle.append(idx)
    
    return where_is_parcicle, cell_content

#------------------------------------------------------------------------------

def neighbours_from_grid(i, where_is_parcicle, cell_content, NEIGHBOURS_LIST):
    
    cell_in_the_center = where_is_parcicle[i]
    cells_around = NEIGHBOURS_LIST[where_is_parcicle[i]]
    
    particles_in_the_center_cell = cell_content[cell_in_the_center]
    particles_around = []
    
    for item in cells_around:
        for jtem in cell_content[item]:
            particles_around.append(jtem)
            
    return particles_in_the_center_cell, particles_around

#------------------------------------------------------------------------------

def particles_to_update_and_neighbours_old(i, where_is_parcicle, cell_content, NEIGHBOURS_LIST, NEIGHBOURS_LIST_2):
    cell_in_the_center = where_is_parcicle[i]
    cells_around_1 = NEIGHBOURS_LIST[where_is_parcicle[i]]
    cells_around_2 = NEIGHBOURS_LIST_2[where_is_parcicle[i]]
    
    particles_in_the_center_cell = cell_content[cell_in_the_center]
    particles_around_1 = []
    particles_around_2 = []
    
    for item in cells_around_1:
        for jtem in cell_content[item]:
            particles_around_1.append(jtem)
            
    for item in cells_around_2:
        for jtem in cell_content[item]:
            particles_around_2.append(jtem)
            
    particles_to_update = particles_in_the_center_cell + particles_around_1
    neighbouring_particles = particles_in_the_center_cell + particles_around_1 + particles_around_2
            
    return particles_to_update, neighbouring_particles

#------------------------------------------------------------------------------

def particles_to_update_and_neighbours(i, where_is_parcicle, cell_content, NEIGHBOURS_LIST, NEIGHBOURS_LIST_2):
    cell_in_the_center = where_is_parcicle[i]
    cells_around_1 = NEIGHBOURS_LIST[cell_in_the_center]
    cells_around_2 = NEIGHBOURS_LIST_2[cell_in_the_center]
    
    # Czyszczenie list zduplikowanych cząsteczek przy użyciu zbiorów
    particles_in_the_center_cell = set(cell_content[cell_in_the_center])
    particles_around_1 = set()
    particles_around_2 = set()
    
    for item in cells_around_1:
        particles_around_1.update(cell_content[item])
            
    for item in cells_around_2:
        particles_around_2.update(cell_content[item])
            
    # Tworzenie list do aktualizacji, eliminując duplikaty
    particles_to_update = list(particles_in_the_center_cell | particles_around_1)
    neighbouring_particles = list(particles_in_the_center_cell | particles_around_1 | particles_around_2)
            
    return particles_to_update, neighbouring_particles

#------------------------------------------------------------------------------

def slab_structure(BZ, INTRANGE):
    ...
    for k in range(0, NZ):
        ...
        for kk in [-1, 1]:
            i_z = k + kk
            NEIGHBOURS.append(i_z)
            
        NEIGHBOURS_2 = [] 
        for kk in [-2, -1, 1, 2]:
            i_z = k + kk
            NEIGHBOURS_2.append(i_z)
                
        NEIGHBOURS_LIST.append(NEIGHBOURS)
        NEIGHBOURS_LIST_2.append(NEIGHBOURS_2)
        counter = counter + 1
    
    return counter, z0, NZ, dim_z, NEIGHBOURS_LIST, NEIGHBOURS_LIST_2

#------------------------------------------------------------------------------

def slab_content(RX, RY, RZ, NZ, dim_z, z0):
    
    where_is_parcicle = []
    slab_content = [[] for _ in range(NZ)]
    
    for i in range(0, len(RX)):
        
        index_z = int((RZ[i] - z0)/dim_z)
        
        slab_content[index_z].append(i)
        where_is_parcicle.append(index_z)
    
    return where_is_parcicle, slab_content

#------------------------------------------------------------------------------

def particles_to_update_and_neighbours_from_slab(i, where_is_parcicle, slab_content, NEIGHBOURS_LIST, NEIGHBOURS_LIST_2):
    slab_in_the_center = where_is_parcicle[i]
    slab_around_1 = NEIGHBOURS_LIST[where_is_parcicle[i]]
    slab_around_2 = NEIGHBOURS_LIST_2[where_is_parcicle[i]]
    
    particles_in_the_center_slab = slab_content[slab_in_the_center]
    particles_around_1 = []
    particles_around_2 = []
    
    for item in slab_around_1:
        for jtem in slab_content[item]:
            particles_around_1.append(jtem)
            
    for item in slab_around_2:
        for jtem in slab_content[item]:
            particles_around_2.append(jtem)
            
    particles_to_update = particles_in_the_center_slab + particles_around_1
    neighbouring_particles = particles_in_the_center_slab + particles_around_1 + particles_around_2
